...
class TestDashboardLogging(unittest.TestCase):
    def setUp(self):
        for mod in [
            "services.monitoring_dashboard.app",
            "services.monitoring_dashboard.db",
            "services.monitoring_dashboard.repository",
        ]:
        ...
        self.mock_jobs = patch(
            "services.monitoring_dashboard.app.get_total_jobs"
        ).start()
        self.mock_models = patch(
            "services.monitoring_dashboard.app.get_total_models"
        ).start()
    ...
    def test_dashboard_uses_existing_correlation_id(self):
        ...
        self.mock_st.session_state.__contains__.side_effect = (
            lambda k: k in session_dict
        )
        self.mock_st.session_state.get.side_effect = lambda k, d=None: session_dict.get(
            k, d
        )
        ...
        self.dashboard.logger.info(
            "dashboard_started", extra={"event": "dashboard_started"}
        )

        logs = self.get_logs()
        logs_with_event = [
            log for log in logs if log.get("event") == "dashboard_started"
        ]
