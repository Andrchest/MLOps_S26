...
class TestTrainingWorkerLogging(unittest.TestCase):
    def setUp(self):
        self.db_pool_patcher = patch(
            "services.training_worker.app.db.db_pool", new=MagicMock()
        )
    ...
    def test_startup_logging(self):
        ...
        db_pool_creating = [
            log for log in log_records if log.get("event") == "db_pool_creating"
        ]
        db_pool_ready = [
            log for log in log_records if log.get("event") == "db_pool_ready"
        ]
        ...
        worker_start = [
            log for log in log_records if log.get("event") == "worker_task_start"
        ]
        self.assertTrue(len(worker_start) > 0, "Worker task start log not found")

    @patch("worker.get_job")
    @patch("worker.process_job")
    async def test_worker_loop_polls_and_processes_job(
        self, mock_process_job, mock_get_job
    ):
        """Test worker loop logging when polling and processing jobs"""
        # Setup mock
        mock_job = {"job_id": 123, "dataset_name": "test_dataset", "dataset_id": 456}
        ...
        worker_started = [
            log for log in log_records if log.get("event") == "worker_loop_started"
        ]
    ...
    async def test_process_job_success_logging(
        ...
        # Test job data
        job = {"job_id": 123, "dataset_name": "test_dataset.csv", "dataset_id": 456}
        ...
        download_logs = [
            log for log in log_records if log.get("event") == "dataset_download_start"
        ]
        ...
        self.assertEqual(success_log["model_version"], "123_456_run_123")
        ...
        mock_update_status.assert_has_calls(
            [call(123, "persisting"), call(123, "succeeded")]
        )

    @patch("worker.download_dataset")
    @patch("worker.subprocess.run")
    @patch("worker.update_status")
    async def test_process_job_failure_logging(
        ...
        # Test job data
        job = {"job_id": 999, "dataset_name": "failing_dataset.csv", "dataset_id": 888}
        ...
        failure_logs = [
            log
            for log in log_records
            if log.get("event") == "training_subprocess_failed"
        ]
        ...
        job_failed_logs = [
            log for log in log_records if log.get("event") == "job_failed"
        ]
    ...
    async def test_worker_loop_error_logging(self, mock_get_job):
        ...
        error_logs = [
            log for log in log_records if log.get("event") == "worker_loop_error"
        ]
    ...
    @patch("worker.subprocess.run")
    @patch("worker.update_status")
    async def test_process_job_warning_logging(
        self, mock_update_status, mock_subprocess_run, mock_download_dataset
    ):
        ...
        with patch("worker.MlflowClient") as mock_mlflow_client, patch(
            "worker.mlflow.get_experiment_by_name"
        ) as mock_get_exp, patch(
            "worker.mlflow.artifacts.download_artifacts"
        ) as mock_download, patch(
            "worker.save_model_to_minio"
        ) as mock_save_minio, patch(
            "worker.save_trained_model"
        ) as mock_save_model, patch(
            "shutil.move"
        ):
            ...
            log_records = self.get_logs()
            warning_logs = [
                log for log in log_records if log.get("event") == "pipeline_warnings"
            ]
