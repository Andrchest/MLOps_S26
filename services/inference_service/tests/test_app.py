...
async def client():
    ...
    with patch("app.db_pool", mock_pool, create=True),          patch("app.model_executor", mock_executor, create=True):
...
async def test_predict_error(mock_save_log, mock_fetch_model, client):
    ...
    with patch("asyncio.get_running_loop") as mock_get_loop:
        ...
        mock_loop.run_in_executor = AsyncMock(side_effect=Exception("Prediction failed"))
        mock_get_loop.return_value = mock_loop
